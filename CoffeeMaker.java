...
public class CoffeeMaker {
	...
	private final int NUM_RECIPES = 3; // The maximum number of recipes as per the requirement is only 3 and not 4;
    ...
    public boolean deleteRecipe(Recipe r) {
        ...
        if(r != null) {
	        for(int i = 0; i < NUM_RECIPES; i++) {
	            if(r.equals(recipeArray[i])) {
	            	// the recipe to be deleted should be reset
	                //recipeArray[i] = recipeArray[i];
	            	recipeArray[i] = new Recipe();
	                // The state of the recipe needs to be updated
	                recipeFull[i] = false;
    ...
    public boolean addInventory(Inventory newInventory) {
        return addInventory(newInventory.getCoffee(), newInventory.getMilk(),
        		newInventory.getSugar(), newInventory.getChocolate());
    }
    /**
    ...
    public int makeCoffee(Recipe r, int amtPaid) {
        ...
        if(canMakeCoffee) {
        	// below line is in the original code.
        	// The issue is that even after the recipe is made, coffee amount in Inventory is added.
        	// instead of it should be reduced.
